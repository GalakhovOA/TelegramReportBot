# Предопределённые ID (замените на реальные Telegram ID)
ALLOWED_MANAGERS = [1039896050]  # Список ID руководителей
ALLOWED_EMPLOYEES = [1039896050]  # Список ID сотрудников

# Шаблоны вопросов для отчёта (последовательные)
QUESTIONS = [
    # Раздел 1: Модель общения
    ...
    # Затем для каждой звезды: ИНН и комментарий (динамически)
    {"key": "ckp_realized", "question": "Реализованные ЦКП (только цифра):"},
    # Затем для каждого ЦКП: ИНН и продукт (динамически)
]
